...
export default function AdminPedidosPage() {
    ...
    async function loadOrders() {
        try {
            ...
            // Note: getAllOrders does getting from DB/Local.
            // We need to resolve images for the thumbnail display if they use idb:
            const processed = await Promise.all(data.map(async (o) => {
                ...
                // Only resolving main images for the board to update fast
                // Details modal resolves its own images
    ...
    function filterOrders() {
        ...
        if (dateFilter !== "todos") {
            ...
            result = result.filter(order => {
                ...
                if (dateFilter === 'hoje') return d.toDateString() === now.toDateString();
                if (dateFilter === 'semana') return d >= new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
                if (dateFilter === 'mes') return d >= new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
                return true;
            });
        }
    ...
    const handleUpdateStatus = async (orderId: string, newStatus: string) => {
        ...
        const success = await updateStatusInDb(orderId, newStatus, "Administrador"); // Assuming admin
        if (success) {
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500">Fluxo unificado de produção</p>
                </div>
            ...
            {selectedOrder && (
                <OrderDetailsModal
                    ...
                    readOnly={false} // Admin has write access
                />
            )}
        </div>
    );
